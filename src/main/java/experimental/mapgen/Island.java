...
public class Island
{
	...
	static double waterLevel = 80;
	static double sandFactor = 1.1;
	static double grassFactor = 1.5;
...
//		JSlider iterationsSlider = new JSlider(1, 16);
//		iterationsSlider.addChangeListener(l -> {
//			iterations = iterationsSlider.getValue();
//			System.out.println("iterations " + iterations);
//			render(img);
//		});
//		content.add(iterationsSlider);
		...
		sandLevelSlider.addChangeListener(l -> {
			...
			System.out.println("sand level " + sandFactor);
			generate(img);
		});
		...
		grassLevelSlider.addChangeListener(l -> {
			...
			System.out.println("grass level " + grassFactor);
			generate(img);
		});
...
//		while (true)
//		{
//			render(img, simplexNoise, iterations, persistence, scale, waterLevel, sandLevel);
//			frame.repaint();
//		}
	}
	...
	private static void generate(BufferedImage img)
	{
		...
		float worldRadius = Math.min(center.x, center.y);
		
		for (int y = 0; y < height; y++)
		{
			for (int x = 0; x < width; x++)
			{
				float centerFactor = 1f - new Vector2(x - center.x, y - center.y).len() / worldRadius;
				
				double h = sumOctave(simplexNoise, iterations, x, y, persistence, scale, 0, 255);
				h *= centerFactor;
				...
				else if (h < waterLevel * grassFactor) img.setRGB(x, y, Color.GREEN.darker().getRGB());
				else img.setRGB(x, y, Color.GRAY.getRGB());
